...
-export([new/0,set_listener/2,add_change/2,remove_change/2,get_change/2,get_all_changes/1,get_min_seq_num/1,get_max_seq_num/1]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2]).

-record(state, { listener = not_set, cache=#{} }).

new() -> gen_server:start_link( ?MODULE, #state{}, []).
% API
set_listener(Pid, L) -> gen_server:cast(Pid,{set_listener,L}).
add_change(Pid,Change) -> gen_server:cast(Pid,{add_change, Change}).
get_change(Pid,{WriterGuid,SequenceNumber}) -> gen_server:call(Pid,{get_change,WriterGuid, SequenceNumber}).

get_all_changes(Pid) -> gen_server:call(Pid,get_all_changes).
remove_change(Pid,{WriterGuid,SequenceNumber}) -> gen_server:cast(Pid,{remove_change, WriterGuid, SequenceNumber}).
get_max_seq_num(Pid) -> gen_server:call(Pid,get_max_seq_num).
get_min_seq_num(Pid) -> gen_server:call(Pid,get_min_seq_num).
% CALL_BACKS
init(S) -> {ok,S}.
...
handle_cast({add_change, Change}, #state{listener = {Pid,Module}}=S) ->
        Module:on_change_available(Pid,{Change#cacheChange.writerGuid,Change#cacheChange.sequenceNumber}),
