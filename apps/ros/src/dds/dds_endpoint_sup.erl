...
init({data_reader, Topic, P_info, Config}) ->
    ...
    SupFlags = #{strategy => one_for_all,
                 intensity => 0,
                 period => 1}, 
    CACHE = #{id => reader_cache,
                 start => {rtps_history_cache, start_link, [Config#endPoint.guid]},
                 restart => permanent,
                 shutdown => 5000,
                 type => worker},
    RTPS_READER = #{id => rtps_reader,
                  start => {rtps_full_reader, start_link, [{P_info, Config}]},
                  restart => permanent,
                  shutdown => 5000,
                  type => worker},
    DDS_DR = #{id => dds_data_reader,
...
init({discovery_reader, P_info, Config}) ->
        ...
        SupFlags = #{strategy => one_for_all,
                     intensity => 0,
                     period => 1}, 
        CACHE = #{id => discovery_reader_cache,       % mandatory
                     start => {rtps_history_cache, start_link, [Config#endPoint.guid]},      % mandatory
                     restart => permanent,   % optional
                     shutdown => 5000, % optional
                     type => worker},   % optional
        RTPS_READER = #{id => discovery_reader,       % mandatory
