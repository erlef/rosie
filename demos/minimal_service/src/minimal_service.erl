...
init(S) -> 
        ...
        Service = ros_node:create_service(Node, example_interfaces_add_two_ints_srv, {?MODULE,self()}),

        {ok,S#state{ros_node=Node, ros_service=Service}}.

handle_call({handle_request,#example_interfaces_add_two_ints_rq{a=A,b=B}},_,S) -> 
        io:format("[ROSIE] Received request for: ~p ~p\n",[A,B]),
        {reply,#example_interfaces_add_two_ints_rp{sum = A + B },S};
