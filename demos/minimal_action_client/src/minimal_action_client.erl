...
handle_cast({on_send_goal_reply, #example_interfaces_fibonacci_send_goal_rp{responce_code=Responce,timestamp=T}}, 
                #state{action_client = AC,goal_info =  #action_msgs_goal_info{goal_id=GOAL_ID}} = S) ->   
        case Responce of
                1 ->  io:format("Goal accepted :)\nRequesting result...\n"), 
                      ros_action_client:get_result(AC, #example_interfaces_fibonacci_get_result_rq{goal_id = GOAL_ID});
