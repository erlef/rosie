...
init(_) -> 
        ...
        Client = ros_node:create_client(Node, example_interfaces_add_two_ints_srv, {?MODULE,self()}),
...
handle_call({ask,{A,B}}, _, #state{add_client=C} = S) -> 
        case ros_client:service_is_ready(C) of
                true -> #example_interfaces_add_two_ints_rp{sum=R} = ros_client:call(C, #example_interfaces_add_two_ints_rq{a = A, b = B}), 
                        {reply, R, S};
