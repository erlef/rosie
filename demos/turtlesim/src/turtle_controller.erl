...
handle_call({spawn_turtle,{X, Y, Angle, Name}}, _, #state{spawn__client=C} = S) -> 
        case ros_client:service_is_ready(C) of
                true ->  #turtlesim_spawn_rp{name=N} = ros_client:call(C, #turtlesim_spawn_rq{x=X, y=Y, theta=Angle, name=Name}),
                         {reply, N, S};
                false -> {reply, server_unavailable, S}
        end;
handle_call(_,_,S) -> {reply,ok,S}.
handle_cast({spawn_turtle_async,{X, Y, Angle, Name}}, #state{spawn__client=C} = S) -> 
        ros_client:cast(C,  #turtlesim_spawn_rq{x=X, y=Y, theta=Angle, name=Name}),
        {noreply,S};
...
handle_info(go, #state{chatter_pub=P} = S) -> 
        Sign = +1,
        Twist = #geometry_msgs_twist{linear=#geometry_msgs_vector3{x=2*Sign,y=0,z=0}},
...
handle_info(right, #state{chatter_pub=P} = S) -> 
        Sign = -1,
        Twist = #geometry_msgs_twist{angular=#geometry_msgs_vector3{x=0,y=0,z=1*Sign}},
        ros_publisher:publish(P,Twist),
        {noreply,S};
