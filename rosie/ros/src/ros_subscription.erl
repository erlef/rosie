...
-record(state,{msg_module, topic_name, qos_profile=#qos_profile{},dds_topic, dds_data_reader, user_process}).



start_link(MsgModule,TopicName, QoSProfile,CallbackHandler) -> 
        gen_server:start_link(?MODULE, #state{msg_module=MsgModule,qos_profile = QoSProfile, topic_name=TopicName, user_process=CallbackHandler}, []).
...
init(#state{msg_module=MsgModule,topic_name=TopicName, qos_profile=QoS}=S) ->
        Subscription = {?MODULE,TopicName},
        pg:join(Subscription, self()),        
        SUB = dds_domain_participant:get_default_subscriber(dds),
        DDS_Topic = #user_topic{type_name = MsgModule:get_type() , name = TopicName,qos_profile=QoS},
