...
-record(state,{topic, listener = not_set, rtps_reader, matched_data_writers = [], history_cache}).
...
handle_cast({on_change_available, ChangeKey}, #state{rtps_reader= GUID, listener = {Name,Module}}=S) -> 
        ...
        {noreply,S};
handle_cast({match_remote_writers,Writers}, #state{matched_data_writers = DW, rtps_reader= Reader} =S) -> 
