...
-export([start_link/1, set_listener/2, add_change/2, remove_change/2, get_change/2,
...
start_link(OwnerGUID) ->
    gen_server:start_link(?MODULE, OwnerGUID, []).
...
handle_info(clean_up_loop, State) ->
    {noreply, State}.

%CALL_BACK HELPERS
h_add_change(#state{cache = C} = State, Change) ->
    State#state{cache =
                    C#{{Change#cacheChange.writerGuid, Change#cacheChange.sequenceNumber} =>
                           Change}}.
...
history_cache_test() ->
    pg:start_link(),
    {ok, _} = rtps_history_cache:start_link(?GUID_UNKNOWN),
