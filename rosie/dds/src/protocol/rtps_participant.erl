...
handle_cast({on_change_available,_}, #state{spdp_reader_guid=R_GUID} = State) -> 
        ...
        LEAVING = [ WGUID || #cacheChange{writerGuid = WGUID, data=#spdp_disc_part_data{status_qos=S}}
                <- CacheContent, ?ENDPOINT_LEAVING(S)],
        [ rtps_history_cache:remove_change({cache_of,R_GUID}, {WGUID,SN}) || #cacheChange{writerGuid = WGUID, sequenceNumber=SN} 
                <- CacheContent, lists:member(WGUID, LEAVING) ],
