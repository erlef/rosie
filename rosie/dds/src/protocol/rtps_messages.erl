...
header(GuidPrefix) -> <<?RTPS,?V_MAJOR,?V_MINOR,?VendorId_0,?VendorId_1,GuidPrefix/binary>>.

is_rtps_packet(<<?RTPS,_:16,_:16,_:(12*8),_/binary>>) -> true;
is_rtps_packet(_) -> false.

parse_rtps_header(<<?RTPS,Version:16,Vendor:16,GuidPrefix:12/binary,PayLoad/binary>>) -> {Version,Vendor,GuidPrefix,PayLoad}.
...
serialize_param_list([{participant_lease,V}| TL],PL) -> P = <<?PID_PARTICIPANT_LEASE_DURATION:16/little,8:16/little,V:32/little,0:32>>, serialize_param_list(TL,[P|PL]);
serialize_param_list([{participant_guid,#guId{prefix=Prefix,entityId=ID}}| TL],PL) -> 
...
serialize_param_list([{builtin_endpoint_set,V}| TL],PL) -> 
        ...
        serialize_param_list(TL,[P|PL]);
serialize_param_list([{default_uni_locator,#locator{kind=K,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}}| TL],PL) -> 
        P = <<?PID_DEFAULT_UNICAST_LOCATOR:16/little, 24:16/little, K:32/little-signed, Port:32/little, 0:(8*12),IP_1:8,IP_2:8,IP_3:8,IP_4:8>>, 
        serialize_param_list(TL,[P|PL]);
serialize_param_list([{default_multi_locator,#locator{kind=K,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}}| TL],PL) -> 
        P = <<?PID_DEFAULT_MULTICAST_LOCATOR:16/little, 24:16/little, K:32/little-signed, Port:32/little, 0:(8*12),IP_1:8,IP_2:8,IP_3:8,IP_4:8>>, 
        serialize_param_list(TL,[P|PL]);
serialize_param_list([{meta_uni_locator,#locator{kind=K,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}}| TL],PL) -> 
        P = <<?PID_METATRAFFIC_UNICAST_LOCATOR:16/little, 24:16/little, K:32/little-signed, Port:32/little, 0:(8*12),IP_1:8,IP_2:8,IP_3:8,IP_4:8>>, 
        serialize_param_list(TL,[P|PL]);
serialize_param_list([{meta_multi_locator,#locator{kind=K,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}}| TL],PL) -> 
        P = <<?PID_METATRAFFIC_MULTICAST_LOCATOR:16/little, 24:16/little, K:32/little-signed, Port:32/little, 0:(8*12),IP_1:8,IP_2:8,IP_3:8,IP_4:8>>, 
        serialize_param_list(TL,[P|PL]);
serialize_param_list([{history_qos,{QOS,Depth}}| TL],PL) -> P = <<?PID_HISTORY:16/little,8:16/little,QOS:32/little,Depth:32/little>>, serialize_param_list(TL,[P|PL]);
serialize_param_list([{durability_qos,V}| TL],PL) -> P = <<?PID_DURABILITY:16/little,4:16/little,V:32/little>>, serialize_param_list(TL,[P|PL]);
serialize_param_list([{reliability_qos,V}| TL],PL) -> P = <<?PID_RELIABILITY:16/little,12:16/little,V:32/little,0:64>>, serialize_param_list(TL,[P|PL]);
serialize_param_list([{topic_name,V}| TL],PL) -> 
        L = length(V),
        Pad = 4  - (length(V) rem 4) + 4,
        P = <<?PID_TOPIC_NAME:16/little,(L+Pad):16/little,(L+1):32/little,(list_to_binary(V))/binary,0:(Pad*8)>>, serialize_param_list(TL,[P|PL]);
serialize_param_list([{topic_type,V}| TL],PL) -> 
        L = length(V),
        Pad = 4  - (length(V) rem 4) + 4,
        P = <<?PID_TYPE_NAME:16/little,(L+Pad):16/little,(L+1):32/little,(list_to_binary(V))/binary,0:(Pad*8)>>, serialize_param_list(TL,[P|PL]);
...
sedp_data_to_param_payload(#sedp_disc_endpoint_data{
        ...
        topic_type = TT, topic_name = TN, endpointGuid = GUID, history_qos = H_QOS
        }) -> 
        list_to_binary(serialize_param_list([
                {sentinel,none},{endpoint_guid,GUID},
                {rtps_version,P_VER},{vendor_id,V_ID},
                {history_qos,H_QOS},{durability_qos,D_QOS},{reliability_qos,R_QOS},
                {topic_type,TT},{topic_name,TN}
        ])).

spdp_data_to_param_payload(#spdp_disc_part_data{
        domainId = D_ID,
        ...
        availableBuiltinEndpoints = B_IN_MASK, % bitmask 4 byte
        builtinEndpointQos = B_IN_END_QOS, % for best_effort data-reader
        key = Key,
        leaseDuration = LEASE
        }) -> 
        list_to_binary(serialize_param_list([
                {sentinel,none},
                {domain_id,D_ID},{rtps_version,P_VER},{vendor_id,V_ID},
                {participant_guid,#guId{prefix=Prefix,entityId=?ENTITYID_PARTICIPANT}},
                {builtin_endpoint_set,B_IN_MASK},{participant_lease,LEASE}
                ]
...
serialize_heatbeat(#heartbeat{writerGUID=#guId{entityId=WID}, readerGUID=#guId{entityId=RID},
        final_flag = FF, min_sn=MinSN, max_sn=MaxSN, count=C}) -> 
        Body = <<(RID#entityId.key):3/binary,(RID#entityId.kind):8,
                (WID#entityId.key):3/binary,(WID#entityId.kind):8,
                0:32/little,MinSN:32/little,0:32/little,MaxSN:32/little,C:32/little>>,
        SubHead = serialize_sub_header(#subMessageHeader{
                kind = ?SUB_MSG_KIND_HEARTBEAT,
                length = byte_size(Body),
                flags = <<0:6, FF:1, 1:1>>
        }),
        %io:format("~p\n",[Body]),
...
serialize_acknack_body( #acknack{writerGUID=#guId{entityId=#entityId{key=WriterID,kind=WriterKind}}, 
                                readerGUID=#guId{entityId=#entityId{key=ReaderID,kind=ReaderKind}}, 
                                final_flag = Final,
                                sn_range = BitMapBase,
                                count = Count}) when is_integer(BitMapBase)-> 
        <<ReaderID:3/binary,ReaderKind:8,
        WriterID:3/binary,WriterKind:8,
        0:32,BitMapBase:32/little,
        0:32/little,Count:32/little>>;
serialize_acknack_body( #acknack{writerGUID=#guId{entityId=#entityId{key=WriterID,kind=WriterKind}}, 
                                readerGUID=#guId{entityId=#entityId{key=ReaderID,kind=ReaderKind}}, 
                                final_flag = Final,
                                sn_range = Range,
        ...
        WriterID:3/binary,WriterKind:8,
        0:32,BitMapBase:32/little,
        NumBits:32/little, (calc_bitmap(BitMapBase,NumBits,Range))/binary,
        Count:32/little>>.
        
bit_in_map(_,_,BitMap,[]) -> BitMap;
...
param_to_record(?PID_PARTICIPANT_LEASE_DURATION, <<Sec:32/little,_:32>>) ->{participant_lease,Sec};
...
param_to_record(?PID_BUILTIN_ENDPOINT_SET, <<P:32/little-unsigned-integer>>) -> {builtin_endpoint_set,P};
param_to_record(?PID_DOMAIN_ID, P) -> {domain_id,P};
param_to_record(?PID_DEFAULT_UNICAST_LOCATOR, <<Kind:32/little,Port:32/little,_:12/binary,IP_1:8,IP_2:8,IP_3:8,IP_4:8>>) ->
        {default_uni_locator,#locator{kind=Kind,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}};
param_to_record(?PID_DEFAULT_MULTICAST_LOCATOR, <<Kind:32/little,Port:32/little,_:12/binary,IP_1:8,IP_2:8,IP_3:8,IP_4:8>>) ->
        {default_multi_locator,#locator{kind=Kind,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}};
param_to_record(?PID_METATRAFFIC_UNICAST_LOCATOR, <<Kind:32/little,Port:32/little,_:12/binary,IP_1:8,IP_2:8,IP_3:8,IP_4:8>>) ->
        {meta_uni_locator,#locator{kind=Kind,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}};
param_to_record(?PID_METATRAFFIC_MULTICAST_LOCATOR, <<Kind:32/little,Port:32/little,_:12/binary,IP_1:8,IP_2:8,IP_3:8,IP_4:8>>) ->
        {meta_multi_locator,#locator{kind=Kind,port=Port,ip={IP_1,IP_2,IP_3,IP_4}}};
param_to_record(?PID_TOPIC_NAME, <<L:32/little,Name:(L-1)/binary,_/binary>>) -> {topic_name,binary_to_list(Name)};
param_to_record(?PID_TYPE_NAME, <<L:32/little,Name:(L-1)/binary,_/binary>>) -> {topic_type,binary_to_list(Name)};
param_to_record(?PID_ENDPOINT_GUID, <<Prefix:12/binary, Key:3/binary, Kind:8>>) -> {endpoint_guid,#guId{prefix=Prefix,entityId=#entityId{key=Key,kind=Kind}}};
param_to_record(?PID_DURABILITY,<<V:32/little>>) -> {durability_qos,V};
param_to_record(?PID_RELIABILITY,<<V:32/little,_:64>>) -> {reliability_qos,V};
param_to_record(?PID_HISTORY,<<QOS:32/little,Depth:32/little>>) -> {history_qos,{QOS,Depth}};
param_to_record(?PID_SENTINEL,P) -> {sentinel,none};
param_to_record(_,_) -> {unknown,none}.
% parameter parsing
parse_param(<<ID:16/little,Length:16/little, Payload/binary>>) -> 
        <<Value:Length/binary, Next/binary>> = Payload,
        {ID, Value, Next}.
parse_param_list(<<>>,L) -> L;
parse_param_list(Payload,L) ->  
        {ID, Value, Next} = parse_param(Payload),
        Entry = param_to_record(ID,Value),
        parse_param_list(Next,L++[Entry]).
parse_param_list(Payload) -> parse_param_list(Payload,[]).


parse_acknack(<<_:6, Final:1,_:1>>, 
                <<ReaderID:3/binary,ReaderKind:8,
                WriterID:3/binary,WriterKind:8,
                _:32,BitMapBase:32/little, 
                0:32,
                Count:32/little>>) -> 
...
parse_acknack(<<_:6,Final:1,_:1>>, 
        ...
        WriterID:3/binary,WriterKind:8,
        _:32,BitMapBase:32/little,
        NumBits:32/little, BitMap_and_count/binary>>) when (NumBits - BitMapBase) =< 250 -> 
        BITMAP_LENGTH = 32 * (NumBits div 32 + 1),
        ...
        #acknack{writerGUID=#guId{entityId=#entityId{key=WriterID,kind=WriterKind}}, 
                readerGUID=#guId{entityId=#entityId{key=ReaderID,kind=ReaderKind}}, 
                final_flag = Final,
                sn_range = filter_by_bits(BitMapBase,Set,BITMAP_LENGTH,lists:seq(BitMapBase,BitMapBase + NumBits)),
                count = Count}.
...
parse_heartbeat(<<_:6,Final:1,_:1>>,
        <<(Reader_key):3/binary,(Reader_kind):8,
        (Writer_key):3/binary,(Writer_kind):8,
        0:32/little,MinSN:32/little,0:32/little,MaxSN:32/little,C:32/little>>) -> 
                #heartbeat{
                ...
                final_flag = Final, min_sn=MinSN, max_sn=MaxSN, count = C}.
...
heartbit_single_test() -> 
        HB = #heartbeat{
                writerGUID=#guId{entityId=?ENTITYID_UNKNOWN},
                min_sn=1,max_sn=1,
                final_flag = 0,
